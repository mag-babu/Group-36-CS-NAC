...
def get_all(conn):
    ...
    df = spark.read.format("jdbc").option("url","jdbc:mysql://localhost:3306/news").option("driver", "com.mysql.cj.jdbc.Driver").option("dbtable", "articles").option("user", "root").option("password", "csproj").load() 
...
def get_records_count(cursor):
    # execute the command
    cursor.execute('''SELECT * FROM nac_news;''')
...
def get_by_unq_serno(cursor, eid):
    # sql query
    query = '''SELECT * FROM nac_news WHERE id = %s;'''
    # execute the command
